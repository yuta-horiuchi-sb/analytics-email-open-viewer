 ...
 */
const InitialsAvatar = ({ firstName, lastName, className }: { firstName: string; lastName: string; className?: string }) => {
                ...
                <td>
                    <div className="status-cell">
                        {interaction.wasOpened 
                            ? <span className="status-badge opened">Opened{interaction.opens.length > 1 && <span className="open-count">({interaction.opens.length}x)</span>}</span> 
                            : interaction.sentTime 
                                ? <span className="status-badge sent">Sent</span> 
                                : <span className="status-badge unknown">Unknown</span>}
                        {isExpandable && <span className={`chevron ${isExpanded ? 'expanded' : ''}`}>&#9654;</span>}
                    </div>
                </td>
            </tr>
                ...
                <tr className="details-row">
                    <td colSpan={2}>
                        <div className="details-container">
                            <h4 style={{ color: '#333', fontSize: '1.15rem', fontWeight: 'bold', paddingBottom: '0.7rem' }}>Interaction Details</h4>
                            {interaction.sentTime && <div className="detail-block"><p><strong>Sent at:</strong> {formatDisplayDateTime(interaction.sentTime)}</p></div>}
...
export const AnalyticsEmailOpenViewer = ({ emailid, domain = "app.staffbase.com", allemailsview = true, emaillistlimit = 100, defaultemailpagesize = 5, defaultrecipientpagesize = 5, enablecsvdownload = false }: AnalyticsEmailOpenViewerProps): ReactElement => {
    ...
    const handleDetailDateChange = (value: string, type: 'since' | 'until') => {
        ...
        if (until.getTime() - since.getTime() > thirtyDaysInMillis) {
            if (type === 'since') until = new Date(since.getTime() + thirtyDaysInMillis);
    ...
    const handleCsvExport = () => {
        ...
        for (const interaction of filteredRecipients) {
            ...
            if (sentTime) csvRows.push([...baseRow, '"Sent"', `"${formatDisplayDateTime(sentTime)}"`, '""'].join(','));
            for (const open of opens) {
                csvRows.push([...baseRow, '"Open"', `"${formatDisplayDateTime(open.openTime)}"`, '""'].join(','));
                for (const click of open.clicks) {
                    csvRows.push([...baseRow, '"Click"', `"${formatDisplayDateTime(click.clickTime)}"`, escapeCsvField(click.targetUrl)].join(','));
                }
            }
        }
    ...
    // These hooks prevent expensive calculations (like sorting and filtering) from re-running on every single render.
    // They only re-calculate when their dependencies (the data they operate on) have actually changed.
        ...
        if (sortConfig.key && sortConfig.direction !== 'original') {
            sortableData.sort((a, b) => {
                ...
                if (sortConfig.key === 'status') {
                    const getStatusScore = (item: RecipientInteraction) => item.wasOpened ? item.opens.length : (item.sentTime ? 0 : -1);
                    return sortConfig.direction === 'descending' ? getStatusScore(b) - getStatusScore(a) : getStatusScore(a) - getStatusScore(b);
                }
                return 0;
            });
        }
    ...
    const filteredRecipients = sortedRecipients.filter(r => 
        `${r.user.firstName} ${r.user.lastName}`.toLowerCase().includes(recipientSearchTerm.toLowerCase())
    );
    ...
    const selectedEmailTitle = useMemo(() => allEmails.find(e => e.id === selectedEmailId)?.title || "Email", [allEmails, selectedEmailId]);
                        ...
                        <>
                                    ...
                                    <div key={email.id} className="email-list-item" onClick={() => handleEmailSelect(email.id)}>
                                        ...
                                        <div className="email-list-item-right">
                                            {email.targetAudience?.totalRecipients !== undefined && (
                                                <span className="recipient-count-pill">{email.targetAudience.totalRecipients} Recipients</span>
                                            )}
                            ...
                            <div className="pagination-controls">
                                ...
                                <div className="page-buttons">
                                    <button onClick={() => setEmailListPage(p => p - 1)} disabled={emailListPage === 0}><FaCaretLeft /></button>
                                    <button onClick={() => setEmailListPage(p => p + 1)} disabled={emailListPage >= emailPageCount - 1}><FaCaretRight /></button>
                                </div>
                            </div>
                        </>
                    ...
                    <div className="detail-view-controls">
                        <div className="filter-and-date-container">
                            <div className="recipient-filter">
                                <input type="text" placeholder="Filter recipients..." value={recipientSearchTerm} onChange={e => { setRecipientSearchTerm(e.target.value); setRecipientPage(0); }} />
                            </div>
                            <div className="date-picker-group">
                                <label>From:</label>
                                <input type="datetime-local" value={toInputDateTimeString(detailSinceDate)} onChange={e => handleDetailDateChange(e.target.value, 'since')} max={nowString} />
                                <label>To:</label>
                                <input type="datetime-local" value={toInputDateTimeString(detailUntilDate)} onChange={e => handleDetailDateChange(e.target.value, 'until')} max={nowString} />
                            </div>
                        </div>
                        <div className="stats-and-export-container">
                            {emailStats && (
                                <div className="email-stats-container">
                                    <div className="stat-item"><span className="stat-value">{emailStats.totalRecipients}</span><span className="stat-label">Recipients</span></div>
                                    <div className="stat-item"><span className="stat-value">{emailStats.uniqueOpens}</span><span className="stat-label">Unique Opens</span></div>
                                    <div className="stat-item"><span className="stat-value">{emailStats.totalOpens}</span><span className="stat-label">Total Opens</span></div>
                                </div>
                            )}
                            {enablecsvdownload && (
                    ...
                    {paginatedRecipients.length > 0 ? (
                        <>
                            <table className="performance-table">
                                ...
                                <tbody>
                                    {paginatedRecipients.map(i => <RecipientRow key={i.user.id} interaction={i} />)}
                                </tbody>
                            </table>
                            <div className="pagination-controls">
                                <div className="page-size-control">
                                    <label>Show:</label>
                                    <select value={recipientsPerPage} onChange={e => { setRecipientsPerPage(Number(e.target.value)); setRecipientPage(0); }}>
                                        <option value={5}>5</option><option value={10}>10</option><option value={20}>20</option><option value={50}>50</option>
                                    </select>
                                </div>
                                <div className="page-buttons">
                                    <button onClick={() => setRecipientPage(p => p - 1)} disabled={recipientPage === 0}><FaCaretLeft /></button>
                                    <button onClick={() => setRecipientPage(p => p + 1)} disabled={recipientPage >= recipientPageCount - 1}><FaCaretRight /></button>
                                </div>
                            </div>
                        </>
